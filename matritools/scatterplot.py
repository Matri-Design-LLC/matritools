...
def scatter_plot(df, ntf, grid_color, key_column, x_column, y_column, z_column, color_column, common_tag, target_keys=None):
    ...
    def bad_input(value, is_clamped, value_min, value_max):
        return str(value) == 'nan' or value is None or (not is_clamped and (value > value_max or value < value_min))


    # main loop
    for index, row in df.iterrows():
        if bad_input(row[x_column], clamp_x, x_column_min, x_column_max) or \
            bad_input(row[y_column], clamp_y, y_column_min, y_column_max):
            continue
        ...
        node.set_tag(row[key_column] + ", " + common_tag + '(' + str(row[x_column]) + ", " + str(round(row[y_column], 2)))
