...
def create_df_from_json(json_file_name: str) -> pd.DataFrame:
	"""
...
def create_df_from_json_string(json_string: str) -> pd.DataFrame:
	"""
...
def interpolate_df_column(df: pd.DataFrame,
	...
	Parameters:
		...
		clamp_value (bool: False) - if value passed is outside of range the scalar function was built with,
...
def make_interpolator(old_min: float,
	...
	"""
	# Error checking
	check_type(old_min, float)
	check_type(old_max, float)
	check_type(new_max, float)
	check_type(new_max, float)
	check_type(handle_error, bool)
	check_type(clamp_value, bool)
	if default_value is not None:
		check_type(default_value, float)
...
def make_df_column_interpolator(column_series: pd.Series,
								new_min: float,
								new_max: float,
								clamp_value: bool = False,
								handle_error: bool = False,
...
def make_set_scalar(column_series: pd.Series, min_value: float, max_value: float):
	"""
...
def separate_compound_dataframe(df,  name_template="") -> List[pd.DataFrame]:
	"""
...
def find_index_in_set(value_set, value) -> int:
	"""
...
def set_to_list(value_set: set) -> list:
	"""
...
def dict_to_multilined_str(dictionary: dict, indent_level: int=0) -> str:
	"""
...
def create_column_value_color_legend(column_series: pd.Series) -> dict:
	"""
...
def split_df_by_value(df: pd.DataFrame, column: str) -> List[pd.DataFrame]:
	"""
...
def check_type(obj: object, passed_type: type, cast: bool = True) -> None:
	"""
...
def get_node_indexes(node_file: str, tag_file: str) -> None:
	"""
